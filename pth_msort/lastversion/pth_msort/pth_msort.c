...
void index_finder(int*arr,int *a,int* b,int Length){
	...
	int x=Length/4;
	i=offset;
	idx1=binarySearch(a,0,Length-1,b[x+i]);
	while(idx1==-1 & b[x+i]<a[0]){
		i++;
	idx1=binarySearch(a,0,Length-1,b[x+i]);
	}
	
	idx2=binarySearch(a,0,Length-1,b[x*2+i]);
	while(idx2==-1 & b[2*x+i]<a[0]){
		i++;
	idx1=binarySearch(a,0,Length-1,b[x+i]);	
	idx2=binarySearch(a,0,Length-1,b[x*2+i]);
	}
	idx3=binarySearch(a,0,Length-1,b[3*x+i]);
	while(idx3==-1 & b[3*x+i]<a[0]){
		i++;
	idx1=binarySearch(a,0,Length-1,b[x+i]);	
	idx2=binarySearch(a,0,Length-1,b[x*2+i]);
	idx3=binarySearch(a,0,Length-1,b[3*x+i]);
...
void* mergelvl1(void* Rank){
	long R=(long )Rank;
	mergesort(x_val,R*(N_val/thread_count),(R+1)*(N_val/thread_count)-1);
...
void* mergelvl3(void* Rank){
	...
	switch (R){
		case 0:
			merge1(x_val,x_sort,left[0]+right[0],left[1]+right[0]-1,left[1]+right[1]-1);
			merge1(x_val,x_sort,left[1]+right[1],left[2]+right[1]-1,left[2]+right[2]-1);	
			break;
		case 1:
			merge1(x_val,x_sort,left[2]+right[2],left[3]+right[2]-1,left[3]+right[3]-1);
			merge1(x_val,x_sort,left[3]+right[3],left[4]+right[3]-1,left[4]+right[4]-1);
			
			break;
		case 2:
			merge1(x_val,x_sort,left[4]+right[4],left[5]+right[4]-1,left[5]+right[5]-1);
			merge1(x_val,x_sort,left[5]+right[5],left[6]+right[5]-1,left[6]+right[6]-1);
			break;
		case 3:	
			merge1(x_val,x_sort,left[6]+right[6],right[6]+N_val/2-1,N_val-1);
			break;	
	}
...
void mergeSortParallel (const int* values, unsigned int N, int* sorted) {
	...
	thread_count=4;
	if(x_sort[0]<x_sort[N/2]){
	index_finder(right,x_sort+N/2,x_sort,N/2);
	index_finder(left,x_sort,x_sort+N/2,N/2);
	}
	else{
	index_finder(left,x_sort,x_sort+N/2,N/2);
	index_finder(right,x_sort+N/2,x_sort,N/2);
