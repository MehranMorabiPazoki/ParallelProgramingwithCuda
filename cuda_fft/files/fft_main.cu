...
void bit_reverse(float* X, int n, float* temp);
// =================================================================================
...
int main(int argc, char *argv[]) {
   ...
   // bit_reverse(X_serial_r, N, tmp_r);
    //bit_reverse(X_serial_i, N, tmp_i);
    clock_t t1 = clock();
...
void bit_reverse(float* X, int n, float* temp) {
	if (n > 1) {	// otherwise, do nothing and return
		...
		bit_reverse(ve, n / 2, X);	// reverse on even-indexed elements of v[]
		bit_reverse(vo, n / 2, X);	// reverse on odd-indexed elements of v[]
